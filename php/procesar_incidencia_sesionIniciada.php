...
if ($_SERVER["REQUEST_METHOD"] !== "POST") {
    header("Location: ../InicioSesion/usuario/contacto.php?error=metodo_no_valido");
    exit();
}
...
if (!empty($camposFaltantes)) {
    header("Location: ../InicioSesion/usuario/contacto.php?error=campos_obligatorios");
    exit();
}

// Verificar política de privacidad
if (!isset($_POST['politica'])) {
    header("Location: ../InicioSesion/usuario/contacto.php?error=politica_requerida");
    exit();
}
...
try {
    ...
    if ($stmt->execute()) {
        header("Location: ../InicioSesion/usuario/contacto.php?success=true");
        exit();
    } else {
    ...
    header("Location: ../InicioSesion/usuario/contacto.php?error=error_bd");
    exit();
} finally {
